...
# F2 write-boundary tests (interface.ready/SAFE_CORE_VERIFIED/chain VERIFIED
# alone being sufficient to authorize a broadcast, and the fix requiring
# independent-operator-group agreement) live in test_write_authorization.py.
# This class used to document the boundary *before* that gate existed; it is
# gone now that Network.broadcast_transaction() enforces one, to avoid
# keeping an assertion around that describes a fixed vulnerability as if it
# were still current behavior.
